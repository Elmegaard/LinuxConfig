...
function generateLabelDropDownItem(params) {
	...
	$item.click({mail:params.mail, $mail:params.$mail}, function(event) {
		...
		if (params.isMove) {
			event.data.mail.moveLabel(params.label.id);
			hideFullEmail();
		} else {
		...
		if (!params.forceDisplayImages && currentlyOpenFullEmail && currentlyOpenFullEmail.id == mail.id && $fullEmail.is(":visible")) {
			...
			mail.getThread(params, function(response) {
				if (response.error) {
					...
					if (mail.hasLabel(GmailAPI.labels.STARRED)) {
...
function renderAccount(account) {
	...
	if (account.error) {
		...
		inboxForStr += "(<span class='accountError'>" + account.getError().niceError + "</span> " + account.getError(true).instructions + ")";
		inboxForStr += "</span>";
	...
	$account.find(".sendPageLink").click({account:account}, function(event) {
		chrome.tabs.getSelected(null, function (tab) {
			...
			var subject = unescape(tab.title);
			var message = unescape(tab.url);
			...
			event.data.account.openCompose({subject:subject, message:message});
...
$(document).ready(function () {
   ...
   setTimeout(function() {
	...
	$("#switchView").mouseup(function(e) {
		...
		// right click
		if (e.button == 2) {
			chrome.tabs.create({url:"http://jasonsavard.com/wiki/Popup_window?ref=GmailPopup"});
			e.preventDefault();
			return false;
		} else {
	...
	$("#followMe").click(function() {
		chrome.tabs.create({url:"http://jasonsavard.com/?followMe=true&ref=GmailCheckerOptionsMenu"});
	});
	...
	$("#shareMenu li").click(function() {
		...
		if (value == "SEP") {
			...
			var urlToShare = "http://jasonsavard.com/checkerPlusForGmail";
			var imageToShare = "http://jasonsavard.com/images/extensions/mediumCheckerPlusForGmail.png";
			...
			if (value == "googlePlus") {
				...
				openWindowInCenter("https://www.facebook.com/dialog/share?app_id=166335723380890&display=popup&href=" + encodeURIComponent(urlToShare) + "&redirect_uri=" + encodeURIComponent("http://jasonsavard.com/tools/closePopup.htm"), '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes', 600, 500);
	...
	$("#title").click(function() {
		...
		if (Settings.read("clickingCheckerPlusLogo") == "openHelp") {
			url = "http://jasonsavard.com/wiki/Checker_Plus_for_Gmail?ref=GmailChecker";
		} else {
			url = MAIL_DOMAIN;
		}
